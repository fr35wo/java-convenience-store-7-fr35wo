...
public class PurchaseItemParser {
    ...
    public List<CartItem> parse(String input, Inventory inventory) {
        ...
        if (input == null || input.trim().isEmpty()) {
            throw new IllegalArgumentException("[ERROR] 구매 항목이 입력되지 않았습니다.");
        }
        ...
        while (matcher.find()) {
            ...
            // 수량이 유효하지 않으면 예외 발생
            if (quantity <= 0) {
                throw new IllegalArgumentException("[ERROR] 수량은 1 이상이어야 합니다: " + productName);
            }
            ...
            // 일반 상품이 있는 경우, 남은 수량만큼 일반 상품을 추가
            else if (regularProduct != null) {
